-- Historical RPC reconciliation already applied to production via Supabase.
-- Added to keep repository and production migration histories aligned.
...
begin
  ...
  values (
    ...
    'draft', original.project_id, 'journal_reversal', original.id,
    original.id, (select auth.uid())
  )
