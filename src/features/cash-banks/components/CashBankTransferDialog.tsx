import { ArrowLeftRight, LoaderCircle, X } from 'lucide-react'
import type { CashBankTransferFormState } from '../types/cash-banks.types'

export function CashBankTransferDialog({ form }: { form: CashBankTransferFormState }) {
  if (!form.isOpen) return null

  return (
    <div className="cash-bank-dialog-backdrop" role="presentation">
      <section className="cash-bank-dialog" role="dialog" aria-modal="true" aria-labelledby="transfer-title">
        <header>
          <div>
            <span>حركة مالية</span>
            <h2 id="transfer-title">تحويل بين الحسابات</h2>
            <p>سيُنقل المبلغ ويُنشأ القيد المتوازن في عملية ذرية واحدة.</p>
          </div>
          <button type="button" onClick={form.close} aria-label="إغلاق" disabled={form.isSaving}>
            <X size={18} />
          </button>
        </header>

        <form
          onSubmit={(event) => {
            event.preventDefault()
            void form.submit()
          }}
        >
          <div className="cash-bank-dialog-grid">
            <label>
              من حساب
              <select
                value={form.value.sourceAccountId}
                onChange={(event) => form.update('sourceAccountId', event.target.value)}
              >
                <option value="">اختر حساب المصدر</option>
                {form.accounts.map((account) => (
                  <option value={account.id} key={account.id}>
                    {account.name} — {account.currentBalance.toLocaleString('ar-EG')} ج.م
                  </option>
                ))}
              </select>
            </label>
            <label>
              إلى حساب
              <select
                value={form.value.destinationAccountId}
                onChange={(event) => form.update('destinationAccountId', event.target.value)}
              >
                <option value="">اختر حساب المستفيد</option>
                {form.accounts.map((account) => (
                  <option value={account.id} key={account.id}>
                    {account.name}
                  </option>
                ))}
              </select>
            </label>
            <label>
              التاريخ
              <input
                type="date"
                value={form.value.transactionDate}
                onChange={(event) => form.update('transactionDate', event.target.value)}
              />
            </label>
            <label>
              المبلغ (ج.م)
              <input
                type="number"
                min="0.01"
                step="0.01"
                inputMode="decimal"
                value={form.value.amount}
                onChange={(event) => form.update('amount', event.target.value)}
              />
            </label>
            <label className="cash-bank-dialog-wide">
              الوصف
              <textarea
                value={form.value.description}
                onChange={(event) => form.update('description', event.target.value)}
              />
            </label>
            <label className="cash-bank-dialog-wide">
              رقم المرجع
              <input
                value={form.value.referenceNumber}
                onChange={(event) => form.update('referenceNumber', event.target.value)}
                placeholder="اختياري"
              />
            </label>
          </div>

          {form.submitted && form.errors.length > 0 && (
            <div className="cash-bank-dialog-errors" role="alert">
              {form.errors.map((error) => (
                <p key={error}>{error}</p>
              ))}
            </div>
          )}
          {form.saveError && (
            <div className="cash-bank-dialog-errors" role="alert">
              {form.saveError}
            </div>
          )}

          <footer>
            <span />
            <button type="button" className="cash-bank-secondary" onClick={form.close}>
              إلغاء
            </button>
            <button type="submit" className="cash-banks-primary" disabled={form.isSaving}>
              {form.isSaving ? (
                <LoaderCircle className="cash-bank-spinner" size={17} />
              ) : (
                <ArrowLeftRight size={17} />
              )}
              {form.isSaving ? 'جارٍ الترحيل...' : 'ترحيل التحويل'}
            </button>
          </footer>
        </form>
      </section>
    </div>
  )
}
