...
const initialValue: ProjectCreateInput = {
  name: '', code: '', client: '', location: '', manager: '', status: 'active',
  contractValue: '', startDate: new Date().toISOString().slice(0, 10), endDate: '', notes: '',
}
...
export function useProjectCreateForm() {
  ...
  const close = () => { setIsOpen(false); setSubmitted(false); setValue(initialValue) }
  const review = () => setSubmitted(true)

  return { isOpen, open: () => setIsOpen(true), close, value, update, submitted, errors, preview, review }
}
